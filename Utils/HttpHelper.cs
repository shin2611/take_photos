...
namespace Utils
{
    public static class HttpHelper
    {
        ...
        public static string SendPostByCodition(string postData, string url, string contentType, ref HttpStatusCode httpStatusCode)
        {
            string resp;
            ...
            try
            {
                using (HttpWebResponse myResponse = (HttpWebResponse)myRequest.GetResponse())
                {
                    httpStatusCode = myResponse.StatusCode;
                    using (Stream respStream = myResponse.GetResponseStream())
                    {
                        using (StreamReader respReader = new StreamReader(respStream))
                        {
            ...
            catch (WebException webEx)
            {
                if (webEx.Response != null)
                {
                    using (HttpWebResponse exResponse = (HttpWebResponse)webEx.Response)
                    {
                        httpStatusCode = exResponse.StatusCode;
                        using (StreamReader sr = new StreamReader(exResponse.GetResponseStream()))
                        {
                            responseXml = sr.ReadToEnd();
                        }
        ...
        public static string PostDataJSON(object obj, string url)
        {
            bool success = false;
            string resp;
            System.Text.ASCIIEncoding encoding = new System.Text.ASCIIEncoding();
            ...
            myRequest.CookieContainer = cookie;
            myRequest.AllowAutoRedirect = false;
            using (StreamWriter streamWriter = new StreamWriter(myRequest.GetRequestStream()))
            {
                string json = JsonSerializer.Serialize(obj);
                NLogLogger.LogInfo("URLPost:" + url + "|json_objData:" + json);
            ...
            {
                NLogLogger.PublishException(webEx);
                if (webEx.Response != null)
                {
                    using (HttpWebResponse exResponse = (HttpWebResponse)webEx.Response)
                    {
